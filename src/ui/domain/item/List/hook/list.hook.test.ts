...
import * as GQL from "./coLocation/List.gql.generated";
import { graphql } from "msw";
import * as tag from "~/ui/util/tag";

describe.concurrent("useItem", () => {
  const renderHookFn = () => renderHook(() => useList(), { wrapper });

  test.concurrent("success", async () => {
    server.use(
      graphql.query(GQL.ListDocument, (_, res, ctx) =>
        res.once(
          ctx.data({
            items: [
              {
  ...
  test.concurrent("failure", async () => {
    server.use(
      graphql.query(GQL.ListDocument, (_, res, ctx) => res.once(ctx.errors([])))
    );
